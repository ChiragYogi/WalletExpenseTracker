...
@AndroidEntryPoint
class TransactionCalenderViewFragment : Fragment() {
    ...
    private fun navigateToEdit(transaction: Transaction) {
        val action = TransactionCalenderViewFragmentDirections
            .actionCalenderViewFragmentToAddTransactionFragment(
                transaction,
    ...
    private fun navigateToDelete(transaction: Transaction) {
        val action = TransactionCalenderViewFragmentDirections.actionGlobalDeleteTransaction(transaction)
        findNavController().navigate(action)
    }
}
